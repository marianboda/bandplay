...
var DOWNLOAD_ROOT = Path.join('.', 'downloads')
...
request(albumUrl, (err, res, body) => {
  ...
  let dirName = Path.join(DOWNLOAD_ROOT, `${album.data.artist} - ${album.data.current.title}`)
  for (let track of album.data.trackinfo) {
    console.log(track.track_num + '. ' + track.title + '    https:' + track.file['mp3-128'])
  }
  downloadTrack(album.data.trackinfo[1], dirName, (a,b) => console.log('done'))
})
